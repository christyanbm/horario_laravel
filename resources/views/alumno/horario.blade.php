...
<script>
    ...
    const maxCreditos = {{ $creditosDisponibles }};
    ...
    if (index === -1) {
        ...
        if (creditosTotales + creditos > maxCreditos) {
            conflictDiv.innerText = `⚠️ No puedes agregar "${nombre}", excedes el límite de créditos (${maxCreditos})`;
            conflictDiv.classList.remove('d-none');
            return;
        }

        // Agregar materia
        seleccionadas.push({id, creditos, nombre, horaInicio, horaFin});
